...
MLP 会先将每张 `28 × 28` 图像展开为包含 784 个特征的一维向量。
...
MLP 是理解全连接层、激活函数、前向传播和反向传播的良好起点。

## CNN 模型

CNN 将图像保持为 `(1, 28, 28)` 的形状，使模型能够学习笔画、边缘和局部空间特征。
...
CNN 是图像分类中最经典的神经网络之一。通过对比两个脚本，可以直观看到 MLP 与 CNN 在输入形状和特征提取方式上的区别。
